...
MAE = 279, что означает, что в среднем абсолютное отклонение предсказаний от истинных значений составляет 279

MAPE = 0.006, что означает, что в среднем абсолютное процентное отклонение предсказаний от истинных значений составляет только 0,67%. Это является очень точным предсказанием, что указывает на хорошую работу нашей модели.

R-squared = 0.978, что говорит о том, что наша модель очень хорошо объясняет изменения в данных и может быть полезна для прогнозирования будущих значений.

Исходя из этих результатов, можно сделать вывод, что модель даёт очень точные предсказания, и может быть использована для прогнозирования будущих значений с высокой точностью.
